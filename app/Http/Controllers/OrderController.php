...
class OrderController extends Controller
{
    ...
    public function confirmOrder(Request $request)
    {
        ...
        // ako je povezan, radi se insert transakcije
        $transactionDATA = array();
        
        if ($loyDATA && $loyDATA->active == 1 && $ulogovan->loy_oj != null && $ulogovan->loy_user_id != null &&  $ulogovan->loy_barcode != null):

            $transactionDATA['apikey'] = $loyDATA->api_key;
            $transactionDATA['p_id'] = $orderDATA['order']['order_id'];
            $transactionDATA['p_orgjed'] = $loyDATA->loy_oj;
            $transactionDATA['p_pan'] = $ulogovan->loy_barcode;
            $transactionDATA['p_user_id'] = $loyDATA->loy_user_id;
            $transactionDATA['p_iznos_racuna'] = $orderDATA['order']['total'];
            $transactionDATA['p_iznos_bez_ak'] = 0;
            $transactionDATA['p_iznos_placen_bp'] = 0;
            $transactionDATA['p_iznos_pp'] = 0;
            $transactionDATA['p_komentar'] = $orderDATA['order']['comment'];

            foreach ($orderDATA['order_items'] as $oKey => $ordITM) {
                $transactionDATA['stavke'][$oKey]['rbr'] = $oKey+1;
                $transactionDATA['stavke'][$oKey]['artikal'] = $ordITM['prod_sku'];
                $transactionDATA['stavke'][$oKey]['kolicina'] = $ordITM['quantity'];
                $transactionDATA['stavke'][$oKey]['popust'] = $ordITM['prod_discount'];
                $transactionDATA['stavke'][$oKey]['iznosMP'] = $ordITM['display_price'];
            }

            $transactionDATA['vauceri'] = '';

            $insertTransaction = AppSpotlight::insertTransaction($loyDATA->api_base_url,$transactionDATA);
